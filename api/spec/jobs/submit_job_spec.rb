...
RSpec.describe SubmitJob, type: :job do
  ...
  example 'simple' do
    validation = create(:validation, user:, db: 'MetaboBank') {|validation|
    ...
    SubmitJob.perform_now validation
    ...
    expect(Dir.glob('**/*', base: submission_dir(validation.submission))).to match_array(%w(
      _base
