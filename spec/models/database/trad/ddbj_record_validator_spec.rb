  ...
  example 'invalid' do
    ...
    expect(obj.validation_details).to contain_exactly(
      ...
      have_attributes(
        code:     'SB-02003',
        severity: 'warning',
        message:  'Undefined feature key "bar" (sequence_id=SEQ_1)'
      ),
      have_attributes(
        code:     'SB-02004',
        severity: 'warning',
        message:  'Undefined qualifier key "baz" (sequence_id=SEQ_1, feature=bar)'
      ),
      have_attributes(
        code:     'SB-02005',
        severity: 'error',
        message:  'Invalid presence of qualifier value for key "baz" (sequence_id=SEQ_1, feature=bar)'
      ),
      have_attributes(
        code:     'SB-02006',
        severity: 'error',
        message:  'Sequence length is zero (id=SEQ_1)'
      ),
      have_attributes(
        code:     'SB-02007',
        severity: 'error',
        message:  'N-only sequence is not allowed (id=SEQ_1)'
      ),
      have_attributes(
        code:     'SB-02008',
        severity: 'error',
        message:  'X-only sequence is not allowed (id=SEQ_1)'
      )
    )
  end
end
