...
class VerifyMultipartUploadJobTest < ActiveJob::TestCase
  ...
  test 'a verified file goes straight into the uploader\'s unassigned files' do
    VerifyMultipartUploadJob.perform_now(@key, 'reads.fastq', 'text/plain', 5, nil, users(:alice).id)

    assert_equal [@key], users(:alice).unassigned_files.blobs.pluck(:key)
  end
  ...
  test 'the Blob is not kept when the file cannot be attached' do
    ...
    User.stub :find_by, alice do
      alice.stub :unassigned_files_attachments, refusing do
