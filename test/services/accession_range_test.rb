...
class AccessionRangeTest < ActiveSupport::TestCase
  test 'a run of numbers is shown as a range' do
    ...
    assert_equal 'SAMD00412919–936', AccessionRange.format((412_919..412_936).map { format('SAMD%08d', it) })
  end
  ...
  test 'a short divergence still shows three digits' do
    assert_equal 'SAMD00412919–920', AccessionRange.format(%w[SAMD00412919 SAMD00412920])
  end

  test 'one accession is shown as itself' do
    assert_equal 'PRJDB42369', AccessionRange.format(%w[PRJDB42369])
  end

  test 'nothing issued has no range' do
    assert_nil AccessionRange.format([])
  end
  ...
  test 'the range is the extremes, not the first and last written' do
    assert_equal 'SAMD00000001–100', AccessionRange.format(%w[SAMD00000050 SAMD00000100 SAMD00000001])
  end
end
