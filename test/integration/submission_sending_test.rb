...
class SubmissionSendingTest < ActionDispatch::IntegrationTest
  # `@request` is the integration session's own ivar — it is replaced with
  # an ActionDispatch::Request the moment a `get` is performed, and a path
  # built from it afterwards names nothing.
  ...
  setup do
    ...
    @submission_request = submission_requests(:st26)
    ...
    @validation = @submission_request.validation || @submission_request.create_validation!(progress: :finished, finished_at: Time.current)
  ...
  test 'a fresh check can be sent' do
    get submission_request_path(@submission_request)
    ...
    assert_enqueued_with job: ApplySubmissionRequestJob do
      post submission_request_submission_path(@submission_request)
    end

    assert_response :no_content
    assert_equal 'waiting_application', @submission_request.reload.status
  end
  ...
  test 'a check older than a day is refused with a sentence, not a 404' do
    ...
    assert_includes response.parsed_body['send_blocked_reason'], 'more than 1 day old'

    assert_no_enqueued_jobs only: ApplySubmissionRequestJob do
      with_exceptions_app { post submission_request_submission_path(@submission_request) }
    end

    assert_conform_schema 422
    assert_includes response.parsed_body['error'], 'more than 1 day old'
    assert_equal 'ready_to_apply', @submission_request.reload.status
  end
  ...
  test 'a request that has been put down says to reopen it' do
    @submission_request.update_columns(closed_at: Time.current)
