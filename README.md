...
| TRD_R0013 | validation | error | The entry disagrees with itself about its length: a location (source or feature) leaving the sequence or running backwards, a declared `length` that does not match the sequence, or a location that cannot be read at all. LOCUS is printed from the length and both the source lines and the REFERENCE span from the locations, so all of them have to agree. In `st26` a source location must additionally be the whole sequence exactly, a patent source being a single full-length one; elsewhere covering part of the entry is legitimate. Never corrected automatically — only the producer knows which one was meant |
