...
| TRD_R0013 | validation | error | The entry disagrees with itself about its length: a source location leaving the sequence, a declared `length` that does not match it, or a location that cannot be read at all. LOCUS is printed from the length and both the source lines and the REFERENCE span from the locations, so all of them have to agree. In `st26` the source location must be the whole sequence exactly, a patent source being a single full-length one; elsewhere a partial source is legitimate and only leaving the sequence is refused. Never corrected automatically — only the producer knows which one was meant |
